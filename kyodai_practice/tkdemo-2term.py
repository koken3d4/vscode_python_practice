...
def do_eq():
    "＝キーが押され時の計算動作、第2項の設定、加算の実施入力中の数値のクリア"
    global second_term
    global result
    global current_number
...
b9 = tk.Button(f, text='9',
               width=3, command=key9)
b0 = tk.Button(f, text='0',
               width=2, command=key0)

bc = tk.Button(f, text='C',
               bg='#ff0000',
               width=2,
               command=clear)
bp = tk.Button(f, text='+',
               width=2, command=plus)
be = tk.Button(f, text='=', command=eq)


# Gridがジオメトリマネージャによるウィジェットの割付
